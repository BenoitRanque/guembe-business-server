...
module.exports = async function validateCheckout ({ user_id }, pgClient) {
  const { rows: unavailableListings } = await pgClient.query(`
    SELECT 1
  ...
  const { rows: outOfStockListing } = await pgClient.query(`
    SELECT 1
    FROM webstore.cart_listing
