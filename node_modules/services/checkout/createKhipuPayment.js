...
module.exports = async function createKhipuPayment ({ payment_id, amount, sale_id, user_id }, pgClient) {
  const { payer_name, payer_email } = await getPayerDetails({ user_id }, pgClient)
  const body = await getPaymentBody({ sale_id }, pgClient)

  return khipu.postPayments({
    ...
    body,
    cancel_url: `https://${process.env.PUBLIC_HOSTNAME}/webstore`,
...
async function getPayerDetails({ user_id }, pgClient) {
  const { rows: [ payer ] } = await pgClient.query(`
    SELECT
...
async function getPaymentBody ({ sale_id }, pgClient) {
